...
export async function down(repository: RepositoryModules) {
  ...
  rawMigrationDir = path.join(this.rootDir, this.rawMigrationFolder).normalize();
  builtMigrationDir = path.join(this.rootDir, this.builtMigrationFolder).normalize();
  ...
  constructor(
    ...
    @inject(MongoDBMigrateComponentBindings.MONGODB_BACKUP_OPTIONS) public backUpOptions: MongoDbBackUpOptions,
  ) { }
   ...
   */
  appendTimestampToFilename(filename: string, extension: string | null = null): string {
    const transformedName = filename.trim().toLowerCase().replace(/_/g, '-');
  ...
  sortFiles(toMigrateFiles: string[] = []) {
    ...
    toMigrateFiles.sort((file1: string, file2: string) => {
      ...
      return file1Part === file2Part
        ? 0
        : (file1Part < file2Part ? -1
          : 1);
    });
  }
  ...
  async backupMongoDb(): Promise<boolean> {
    return new Promise((resolve, reject) => {
      ...
      const ls = spawn("sh", ['-c', `mongodump -o ${pathForDbBackup} --uri=${DATASOURCE_URL}`]);
   ...
   */
  async getRepositories(app: Application, allRepositories: object): Promise<RepositoryModules> {
